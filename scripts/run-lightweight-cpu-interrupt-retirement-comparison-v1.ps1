...
# CPU interrupt retirement comparison v1: accepted 9be9fee engine with shipped locality CPU versus
# Copper68k deferred interrupt-sample guard/helper. Timing, placement, host policy, six-pair design and
# workload identities are unchanged from live-WORDSYNC comparison v1.
# See docs/engine/CPU_INTERRUPT_RETIREMENT_2026-09-22.md.
...
public static class CpuInterruptRetirementTopologyV1 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        '8E390EE8307DA4B59FF5C6B9A3E5FCF3A274FDC72E6B51D3211ADC9106C316C5') {
        throw 'This comparison requires the frozen engine shared by reference and retirement candidate.'
    }
    ...
    if ((Get-FileHash "$candidate/Copper68k.dll").Hash -ne
        ...
        throw 'Candidate CPU differs from the validated interrupt-retirement binary.'
    }
    ...
    $topology = @([CpuInterruptRetirementTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL cpu-interrupt-retirement-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
