...
# Blitter-modulo comparison v1: accepted 9bdc53a engine versus final-row modulo
# correction. Same v5 timing, host policy and six-pair design.
# Synthetic identities must match across builds. Native identities are pinned
# separately for the hardware correction documented in BLITTER_FINAL_MODULO.md.
...
public static class BlitterModuloTopologyV1 {
...
$results = @()
$nativeFingerprints = @{
    R = 'engine=lightweight-a500 workload=image cpuMode=conservative-batch warmup=10920 frames=3600 completed=14520 cycle=2063189662 cpu=0x249E90223C45891F hardware=0xF4951BBFA89CA99A output=0x45AB46F7600E1C9F pixels=284204 audioSamples=1924 pcm=real allocated=0 adf=True unsupported=none'
...
try {
    ...
    if ((Get-FileHash "$reference/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the accepted 9bdc53a reference.'
    }
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the validated final-row modulo candidate.'
    }
    ...
    $topology = @([BlitterModuloTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL blitter-modulo-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; native identities differ by documented hardware correction"
