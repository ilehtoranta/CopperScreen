...
# OCS absent-DENISEID comparison v6: pre-change engine versus cached bus word, fixed-size DMA word access and inline register bank.
# Frozen reference and SDK 10.0.401 register candidate both use explicitly pinned .NET 10.0.12.
...
public static class OcsIdentificationTopologyV6 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the frozen inline-register absent-DENISEID candidate.'
    }
    ...
    $topology = @([OcsIdentificationTopologyV6]::Read() | ForEach-Object {
    ...
    "PROTOCOL ocs-identification-comparison-v6; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
