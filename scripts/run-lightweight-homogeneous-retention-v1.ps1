...
# Independent homogeneous-host v1 protocol. Historical hybrid protocols are unchanged.
...
try {
    ...
    "PROTOCOL homogeneous-retention-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload"
    ...
    foreach($workload in $workloads) {
        foreach($label in $order) {
            ...
            try {
                ...
                if($workload.Name -eq 'lemmings' -and ($line -notmatch 'warmup=10920 frames=3600 completed=14520 ' -or
                    $line -notmatch 'cycle=2063321634 cpu=0x6AE7090DA8AFB7E3 hardware=0x334A819F6FDFB1CA output=0xC65F87325946E5DA pixels=284204 audioSamples=1922')) {
                    throw 'Native replay differs from independently validated regression expectations.'
                }
                ...
                $prior=@($results | Where-Object Workload -eq $workload.Name)
    ...
    'VALID measured series; engine throughput only, not presentation/device pacing. No historical evidence replaced.'
} catch {
