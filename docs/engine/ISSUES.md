...
| LWA-CPU-001 | 68000 trace exception in pinned Copper68k | Fixed in development pin | `1.4.1-trace.1` passed scalar/batched vector-9 integration and Thunderbolt protection. The current `1.4.1-locality.1` development pin retains that correction and adds the validated prefetch-locality optimization. Source belongs to CopperMod; packages are distributed as GitHub prereleases. See [CPU_TRACE.md](CPU_TRACE.md) for restore requirements, architecture and coverage. |
