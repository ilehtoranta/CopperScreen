...
# Chip-mirror comparison v1: accepted 1db2d0d versus the frozen Copper restart
# plus A500 memory-map candidate. Same v5 timing, host policy and six-pair design.
# Synthetic identities must match across builds. Native identities are pinned
# separately for the hardware correction documented in SUPER_CARS_II_INVESTIGATION.md.
...
public static class ChipMirrorTopologyV1 {
...
$results = @()
$nativeFingerprints = @{
    R = 'engine=lightweight-a500 workload=image cpuMode=conservative-batch warmup=10920 frames=3600 completed=14520 cycle=2063189682 cpu=0xBCF441F9BABF1113 hardware=0x206DA179786FCB2A output=0xF7328C7C24582FCB pixels=284204 audioSamples=1924 pcm=real allocated=0 adf=True unsupported=none'
    C = 'engine=lightweight-a500 workload=image cpuMode=conservative-batch warmup=10920 frames=3600 completed=14520 cycle=2063189662 cpu=0x249E90223C45891F hardware=0xF4951BBFA89CA99A output=0x45AB46F7600E1C9F pixels=284204 audioSamples=1924 pcm=real allocated=0 adf=True unsupported=none'
}
...
try {
    ...
    if ((Get-FileHash "$reference/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the accepted 1db2d0d reference.'
    }
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the validated chip-mirror candidate.'
    }
    ...
    $topology = @([ChipMirrorTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL chip-mirror-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; native identities differ by documented hardware correction"
