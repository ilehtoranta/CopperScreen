...
# OCS absent-DENISEID comparison v7: pre-change engine versus cached bus word, fixed-size DMA word access and fixed register/palette bounds with original array storage.
# Frozen reference and SDK 10.0.401 fixed-bounds candidate both use explicitly pinned .NET 10.0.12.
...
public static class OcsIdentificationTopologyV7 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the frozen fixed-bounds absent-DENISEID candidate.'
    }
    ...
    $topology = @([OcsIdentificationTopologyV7]::Read() | ForEach-Object {
    ...
    "PROTOCOL ocs-identification-comparison-v7; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
