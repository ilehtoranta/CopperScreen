...
# Lotus CPU bus-spacing comparison v1. Six balanced pairs per complete workload,
# native warmup extended to 14520 for the independently validated input script.
# homogeneous CPU/SMT placement, Normal priority and host-load policy v2.
# Reference is the frozen pre-Lotus engine; candidate is the four-clock bus fix.
# Per-build identities are independently validated against scalar execution and
# native gameplay captures before timing. No historical fingerprint is rebased.
...
if ((Get-FileHash "$PSScriptRoot/agnus-g6-host-load.ps1").Hash -cne '7E1F03FC09E80199B8A18BB32781C967C5035300E5C79B1227EA0DD5F8F1D8C6') {
    throw 'Unvalidated host-load policy helper.'
}
...
public static class LotusBusTimingTopologyV1 {
...
$expectedWorkloadFingerprints = @{
    R = @{
        ...
        lemmings = 'engine=lightweight-a500 workload=image cpuMode=conservative-batch warmup=14520 frames=3600 completed=18120 cycle=2574756864 cpu=0xC815695C1AAE835D hardware=0xD5E0FB6D7BEB0984 output=0xDA8F27A2904E84D4 pixels=284204 audioSamples=1924 pcm=real allocated=0 adf=True unsupported=none'
    }
    C = @{
        lores = 'engine=lightweight-a500 workload=paula-dma-wide cpuMode=conservative-batch warmup=600 frames=7200 completed=7800 cycle=1108395730 cpu=0x61B8E8E422AA0567 hardware=0x2F1CDAE2DDCFA737 output=0x516B826B90FE2CD7 pixels=284204 audioSamples=1924 pcm=real allocated=0 adf=False unsupported=none'
        hires = 'engine=lightweight-a500 workload=paula-dma-wide-hires cpuMode=conservative-batch warmup=600 frames=7200 completed=7800 cycle=1108395726 cpu=0x9453D8C5C72902D3 hardware=0x09554374C200BE7A output=0x342F83A04C3EBCF6 pixels=284204 audioSamples=1924 pcm=real allocated=0 adf=False unsupported=none'
        lemmings = 'engine=lightweight-a500 workload=image cpuMode=conservative-batch warmup=14520 frames=3600 completed=18120 cycle=2574756850 cpu=0x505451D28DF1C053 hardware=0x20084440BFE4BC5A output=0x9DE548805866DC63 pixels=284204 audioSamples=1922 pcm=real allocated=0 adf=True unsupported=none'
    }
}
function Assert-WorkloadFingerprint([string]$Role, [string]$Workload, [string]$Fingerprint) {
    if (!$expectedWorkloadFingerprints.ContainsKey($Role) -or
        !$expectedWorkloadFingerprints[$Role].ContainsKey($Workload) -or
        $Fingerprint -cne $expectedWorkloadFingerprints[$Role][$Workload]) {
        throw "Workload differs from the independently validated complete identity: $Role $Workload"
    }
}
try {
    ...
    $pinnedInputs = @{
        ...
        $inputScript = '2A02229461094001E2BE4A65E3114338CF96A1C5003CD59C532927C586CE44BA'
        ([IO.Path]::GetFullPath('../../media/lemmings-disk2.zip', (Split-Path $inputScript -Parent))) = '0C1CB9C49CBC29B56D2F974A90C510752E256148F8E8CC10CFBD026DB73DEA1A'
    }
    foreach ($path in $pinnedInputs.Keys) {
        if ((Get-FileHash -LiteralPath $path).Hash -cne $pinnedInputs[$path]) {
            throw "Unvalidated native workload input: $path"
        }
    }
    ...
    if ((Get-FileHash "$reference/CopperMod.Amiga.Lightweight.dll").Hash -cne '7EB9C87905BD8CA790DFF21BB918A0B34F7E4B94FE8489F673058BC4D1581FDC') {
        throw 'Unvalidated reference CopperMod.Amiga.Lightweight.dll.'
    }
    if ((Get-FileHash "$reference/CopperMod.Amiga.Lightweight.pdb").Hash -cne '27A3DE15F84AAB332B5C69CAF7A293B3524E180D3D7113F19D20A8A30C91D830') {
        throw 'Unvalidated reference CopperMod.Amiga.Lightweight.pdb.'
    }
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -cne '7EE0B872B7E2F4A491FCBBC4D8BDC2BBFD577DF34A1EBB62CF4B0148018EE3E9') {
        throw 'Unvalidated candidate CopperMod.Amiga.Lightweight.dll.'
    }
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.pdb").Hash -cne 'BAA6E641520837DCB3C8A89DA3938223DEF0D31F0A395F9E2BFBDC04165E6CDF') {
    ...
    foreach ($role in @('R','C')) {
        foreach ($name in $expectedWorkloadFingerprints[$role].Keys) {
            $good = $expectedWorkloadFingerprints[$role][$name]
            Assert-WorkloadFingerprint $role $name $good
            $otherRole = if ($role -eq 'R') { 'C' } else { 'R' }
            $badValues = @($good.Replace('allocated=0','allocated=1'),
                $good.Replace('pixels=284204','pixels=1'), $good.Replace('pcm=real','pcm=none'))
            if ($expectedWorkloadFingerprints[$otherRole][$name] -cne $good) {
                $badValues += $expectedWorkloadFingerprints[$otherRole][$name]
            }
            foreach ($bad in $badValues) {
                $rejected = $false
                try { Assert-WorkloadFingerprint $role $name $bad } catch { $rejected = $true }
                if (!$rejected) { throw "Workload identity negative probe was accepted: $role $name" }
            }
        }
    }
    'VALIDATION all per-build identities positive/negative probes passed before timing; no identity learned from timing samples.'
    $topology = @([LotusBusTimingTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL lotus-bus-timing-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all three full workload identities pinned before timing"
    ...
    $workloads=@(
        ...
        @{Name='lemmings';Flags=@('--rom',$rom,'--adf',$adf,'--input-script',$inputScript,'--warmup','14520','--frames','3600')}
    )
    ...
    foreach($workload in $workloads) {
        foreach($label in $order) {
            ...
            try {
                ...
                Assert-WorkloadFingerprint $role $workload.Name $fingerprint
                $prior=@($results | Where-Object {
                    $_.Workload -eq $workload.Name -and
                    $_.Label.StartsWith($role)
                })
                if($prior.Count -and $prior[0].Fingerprint -ne $fingerprint) { throw 'Repeated within-build fingerprints differ.' }
    ...
    foreach($workload in $workloads) {
        ...
        $disposition=if($regression -gt 1){'FAIL'}elseif($upperPercent -gt 1){'INCONCLUSIVE'}else{'PASS'}
    ...
    'VALID measured series; Lotus CPU bus-spacing throughput comparison. Each complete identity matches its independently validated build. PASS requires every upper bound at most 1%; no exceptions. No historical evidence replaced.'
} catch {
