...
# OCS absent-DENISEID comparison v8: pre-change engine versus cached bus word, fixed-size DMA word access, fixed register/palette bounds and scoped scalar-local initialization.
# Frozen reference and SDK 10.0.401 scoped-local candidate both use explicitly pinned .NET 10.0.12.
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the frozen scoped-local absent-DENISEID candidate.'
    }
    ...
    "PROTOCOL ocs-identification-comparison-v8; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
