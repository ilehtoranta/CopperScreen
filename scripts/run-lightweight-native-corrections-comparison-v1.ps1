...
# Combined native compatibility corrections comparison v1.
# Frozen pre-investigation engine versus CPU byte WORDSYNC alignment,
# finite drive-end-stop recovery and live area-blitter pointer writes together.
# Identical frozen runner/dependencies, stable Copper68k 1.4.1, .NET 10.0.12,
# six balanced pairs, full retained identities and homogeneous-host load policy v2.
...
public static class NativeCorrectionsTopologyV1 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -cne '7EB9C87905BD8CA790DFF21BB918A0B34F7E4B94FE8489F673058BC4D1581FDC') {
        throw 'Unvalidated candidate CopperMod.Amiga.Lightweight.dll.'
    }
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.pdb").Hash -cne '27A3DE15F84AAB332B5C69CAF7A293B3524E180D3D7113F19D20A8A30C91D830') {
    ...
    $topology = @([NativeCorrectionsTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL native-corrections-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all three full workload identities pinned before timing"
    ...
    'VALID measured series; combined native compatibility corrections throughput only. All complete identities match the accepted engine baseline. No historical evidence replaced.'
} catch {
