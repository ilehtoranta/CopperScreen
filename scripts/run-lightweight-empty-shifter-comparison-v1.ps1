...
# Empty-shifter comparison v1: accepted 9be9fee engine versus zero-state
# bitplane-shift fast path. Timing, placement, host policy, six-pair design and
# workload identities are unchanged from live-WORDSYNC comparison v1.
# See docs/engine/EMPTY_SHIFTER_OPTIMIZATION_2026-09-22.md.
...
public static class EmptyShifterTopologyV1 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        '25FC088CEEA8BFB738279A93277DE848DFD29675798BCD8792802AD3ACA72333') {
        throw 'This comparison requires the validated empty-shifter candidate.'
    }
    ...
    $topology = @([EmptyShifterTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL empty-shifter-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
