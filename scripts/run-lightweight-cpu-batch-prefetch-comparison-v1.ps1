...
# CPU batch prefetch comparison v1: accepted 9be9fee engine with shipped locality CPU versus
# Copper68k combined retirement/batch-prefetch helpers. Timing, placement, host policy, six-pair design and
# workload identities are unchanged from live-WORDSYNC comparison v1.
# See docs/engine/CPU_BATCH_PREFETCH_2026-09-23.md.
...
public static class CpuBatchPrefetchTopologyV1 {
...
try {
    ...
    if ((Get-FileHash "$reference/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the accepted 9be9fee reference.'
    }
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        '8E390EE8307DA4B59FF5C6B9A3E5FCF3A274FDC72E6B51D3211ADC9106C316C5') {
        throw 'This comparison requires the frozen engine shared by reference and batch-prefetch candidate.'
    }
    ...
    if ((Get-FileHash "$candidate/Copper68k.dll").Hash -ne
        '881982995B5FBA962E53BBDC83DD64E4893C7B5699802E0C3D31DC33C022DE13') {
        throw 'Candidate CPU differs from the validated batch-prefetch binary.'
    }
    ...
    $topology = @([CpuBatchPrefetchTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL cpu-batch-prefetch-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
