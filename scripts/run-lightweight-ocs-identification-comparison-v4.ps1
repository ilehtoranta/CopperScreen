...
# OCS absent-DENISEID comparison v4: pre-change engine versus cached bus word and fixed-size DMA word access.
# Both unchanged frozen builds use explicitly pinned .NET 10.0.12 after the host update.
...
public static class OcsIdentificationTopologyV4 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the frozen direct-word absent-DENISEID candidate.'
    }
    ...
    $topology = @([OcsIdentificationTopologyV4]::Read() | ForEach-Object {
    ...
    "PROTOCOL ocs-identification-comparison-v4; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
