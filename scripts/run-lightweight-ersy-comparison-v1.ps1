...
# ERSY correction comparison v1. Same six-pair timing/host policy as homogeneous
# retention v1; native identities are independently recorded per build in
# docs/engine/BEAM_SYNC.md. This is a changed-state comparison, not a claim that
# pre/post-correction native states match. Historical protocols remain unchanged.
...
public static class HomogeneousTopologyV1 {
...
$results = @()
$nativeFingerprints = @{
    R = 'engine=lightweight-a500 workload=image cpuMode=conservative-batch warmup=10920 frames=3600 completed=14520 cycle=2063321634 cpu=0x6AE7090DA8AFB7E3 hardware=0x334A819F6FDFB1CA output=0xC65F87325946E5DA pixels=284204 audioSamples=1922 pcm=real allocated=0 adf=True unsupported=none'
    C = 'engine=lightweight-a500 workload=image cpuMode=conservative-batch warmup=10920 frames=3600 completed=14520 cycle=2063189682 cpu=0xBCF441F9BABF1113 hardware=0x206DA179786FCB2A output=0xF7328C7C24582FCB pixels=284204 audioSamples=1924 pcm=real allocated=0 adf=True unsupported=none'
}
...
try {
    ...
    if ((Get-FileHash "$reference/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the accepted 67c13f6 reference.'
    }
    ...
    foreach ($role in @('R', 'C')) {
        ...
        foreach ($bad in @($nativeFingerprints[$otherRole],
    ...
    $topology = @([HomogeneousTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL ersy-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; native identities differ by documented hardware correction"
