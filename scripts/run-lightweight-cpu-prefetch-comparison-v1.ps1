...
# CPU prefetch boundary comparison v1: accepted 9be9fee engine versus one
# Copper68k helper inlining boundary. Timing, placement, host policy, six-pair design and
# workload identities are unchanged from live-WORDSYNC comparison v1.
# See docs/engine/CPU_PREFETCH_BOUNDARY_2026-09-22.md.
...
public static class CpuPrefetchBoundaryTopologyV1 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        '8E390EE8307DA4B59FF5C6B9A3E5FCF3A274FDC72E6B51D3211ADC9106C316C5') {
        throw 'This comparison requires the validated CPU prefetch boundary candidate.'
    }
    ...
    if ((Get-FileHash "$candidate/Copper68k.dll").Hash -ne
        ...
        throw 'Candidate CPU differs from the fully tested helper-boundary binary.'
    }
    ...
    $topology = @([CpuPrefetchBoundaryTopologyV1]::Read() | ForEach-Object {
    ...
    "PROTOCOL cpu-prefetch-boundary-comparison-v1; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
