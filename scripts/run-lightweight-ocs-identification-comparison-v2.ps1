...
# OCS absent-DENISEID comparison v2: pre-change engine versus a single DMA word latch using existing completion histories.
...
public static class OcsIdentificationTopologyV2 {
...
try {
    ...
    if ((Get-FileHash "$candidate/CopperMod.Amiga.Lightweight.dll").Hash -ne
        ...
        throw 'This comparison requires the frozen single-latch absent-DENISEID candidate.'
    }
    ...
    $topology = @([OcsIdentificationTopologyV2]::Read() | ForEach-Object {
    ...
    "PROTOCOL ocs-identification-comparison-v2; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; all workload identities identical across builds"
