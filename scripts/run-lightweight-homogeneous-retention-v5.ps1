...
# Homogeneous retention v5: accepted 1db2d0d reference and post-ERSY native
# identity. Timing, host policy, inputs and six-pair design are unchanged.
# Reference/candidate complete-workload fingerprints must match in every workload.
...
public static class HomogeneousTopologyV5 {
...
try {
    ...
    $topology = @([HomogeneousTopologyV5]::Read() | ForEach-Object {
    ...
    "PROTOCOL homogeneous-retention-v5; Normal; CPU=$LogicalProcessor; protected=$($protected -join ','); six balanced pairs/workload; matching complete-workload identities"
