...
| LWA-NATIVE-001 | North & South CP release / Copper restart with DMA off | Corrected; combined candidate accepted 2026-09-21 | Defer the DMA-disabled frame restart's pointer load until its retained bus output. Untouched CP menus/borders and four hardware-photo probe patterns now agree. Earlier release-blame inference withdrawn; see [investigation](NORTH_SOUTH_INVESTIGATION.md). Combined performance acceptance is recorded with Super Cars II. |
| LWA-NATIVE-002 | Super Cars II / CPU chip-RAM mirrors | Boot Guru corrected; bounded driving verified; accepted 2026-09-21 | Reaches race 1, responds to driving input and completes one lap. Valid six-pair upper bounds: lores +2.2310% (scoped owner exception), hires +0.2671%, native +0.6240% (both pass). See [investigation](SUPER_CARS_II_INVESTIGATION.md). Disk 2/full race remain unverified. |
...
| LWA-NATIVE-005 | Lotus III / disk-2 prompt | CPU bus-spacing correction; driving demos verified; scoped performance exception accepted | The existing Copper68k timing contract prevents adjacent-CCK CPU transfers. Untouched disks and input now pass the prompt; all 72 normal/scalar files through 18,000 fields match. See [correction](LOTUS_III_CPU_BUS_TIMING_2026-09-25.md) and [original diagnosis](LOTUS_III_INVESTIGATION_2026-09-25.md). Player-controlled racing remains unverified. [Performance verification](LOTUS_III_PERFORMANCE_2026-09-25.md) is valid; the owner accepted its exact lores/hires bounds on 2026-09-25. Statistical dispositions and the default 1% limit remain unchanged. |
...
| LWA-CPU-001 | 68000 trace exception in pinned Copper68k | Fixed in stable pin | Stable Copper68k `1.4.1` retains the trace correction and validated locality changes. `1.4.1-trace.1` passed scalar/batched vector-9 integration and Thunderbolt protection. Source belongs to CopperMod; the current package is on NuGet.org. See [CPU_TRACE.md](CPU_TRACE.md) for package history, architecture and coverage. |
| LWA-CPU-002 | CPU bus-transfer spacing / prefetch order | Four-clock bus spacing corrected; instruction ordering OPEN | Lightweight now supplies the existing timing interface instead of treating a one-CCK grant as the complete CPU bus cycle. Two regressions fail before and pass after; 728 engine cases pass. WinUAE comparisons still differ in CLR/MOVE prefetch placement and JSR target-fetch/stack-write order. Resolve those in the shared CPU owner using independent traces; see [evidence](LOTUS_III_CPU_BUS_TIMING_2026-09-25.md). |
