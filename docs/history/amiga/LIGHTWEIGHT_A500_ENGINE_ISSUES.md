...
validation. Legacy selection and CopperBench launch are unavailable in the
...
- **Other workload/performance:** complete Lemmings replay retains previous
  ...
  paired regression result. At the time of this historical record,
  Legacy/default routing was unchanged; current routing is Lightweight by
  default, with unsupported Legacy reported unavailable.
...
- Historical arbitration execution plan: G5L disk evidence and inherited fixes
  is not included in this repository; current disk-arbitration status is recorded
  in LWA-DISK-001 through LWA-DISK-010 in this issue register.
